            ...
            <div  key={page.toString()} className='leaderboard_container'>
                <div className='leaderbaord_page'>
                    <div className='leaderboard_table table1'>
                        ...
                        {leaderboard ? leaderboard.slice(page[0],(page[1]-10)).map(data => <Row rank={data.place} teamname={data.teamname} points={data.points} elims={data.elims} wins={data.wins} avg_place={data.avg_place}/>) : ''}
                    </div>
                    <div className='leaderboard_table table2'>
                        ...
                        {leaderboard ? leaderboard.slice(page[0]+5,page[1]-5).map(data => <Row rank={data.place} teamname={data.teamname} points={data.points} elims={data.elims} wins={data.wins} avg_place={data.avg_place}/>) : ''}
                    </div>
                    <div className='leaderboard_table table3'>
                        <div className='header_container'>
                            <div className='rank_header' onClick={previousPage} >TOP</div>
                            <div className='name_header'>JOUEUR</div>
                            <div style={{fontSize: '13px'}} className='info_header'>PLACE MOY.</div>
                            <div className='info_header'>ÉLIM.</div>
                            <div className='info_header'>WINS</div>
                            <div onClick={nextPage} className='info_header'>PTS</div>
                        </div>
                        {leaderboard ? leaderboard.slice(page[0]+10,page[1]).map(data => <Row rank={data.place} teamname={data.teamname} points={data.points} elims={data.elims} wins={data.wins} avg_place={data.avg_place}/>) : ''}
                    </div>
                </div>
            </div>
        </div>
